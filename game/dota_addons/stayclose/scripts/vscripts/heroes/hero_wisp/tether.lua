...
function CastTether( keys )
	...
	TrackCurrentHealth( keys )
	TrackCurrentMana( keys )
	...
	if distToAlly >= keys.latchdistance then
		ability:ApplyDataDrivenModifier( caster, caster, keys.latchmodifier, {} )
	end
end
...
function CheckDistance( event )
	...
	-- pull back
	FindClearSpaceForUnit(ability.tether_ally, 
		caster:GetAbsOrigin() + ( ability.tether_ally:GetAbsOrigin() - caster:GetAbsOrigin() ):Normalized() * event.radius,
		true)
end
